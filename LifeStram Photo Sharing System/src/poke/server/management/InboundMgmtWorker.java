...
public class InboundMgmtWorker extends Thread {
	...
	public InboundMgmtWorker(ThreadGroup tgrp, int workerId) {
		...
		//if (ManagementQueue.outbound == null)     commented by Krunal
		if (ManagementQueue.inbound == null)
		...
		while (true) {
			...
			try {
				...
				if (req.hasBeat()) {
					...
					ManagementQueue.heartBeatLog.put(req.getBeat().getNodeId(), true);
					logger.info("The HEARTBEATLOG ::"+ManagementQueue.heartBeatLog);
					// TOOD update map
