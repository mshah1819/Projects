...
public class ClientHandler extends SimpleChannelUpstreamHandler {
	...
	public boolean send(GeneratedMessage msg) {
		...
		System.out.println("Writing to the outound channel from the client");
	...
	public void handleMessage(eye.Comm.Response msg) {
		if (msg.getHeader().getRoutingId() == Header.Routing.FINGER) {
			...
			eye.Comm.Response response = (eye.Comm.Response) msg;
			List<ByteString> arrImage = response.getImageList();

			try {
				for (int i = 1; i < arrImage.size(); i++) {
					String fileDestLocation = "/home/krunal/Pictures/"
							+ "DSC.jpg";
					Jab.byteToImage(arrImage.get(i).toByteArray(),
							fileDestLocation);
				}
	...
	public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) {
		logger.info("BINGO! message received in the client handler");
