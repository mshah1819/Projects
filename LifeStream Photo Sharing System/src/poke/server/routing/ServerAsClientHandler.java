...
public class ServerAsClientHandler extends SimpleChannelUpstreamHandler {
	protected static Logger logger = LoggerFactory.getLogger("serverAsClient");

	private volatile Channel channel;

	public ServerAsClientHandler() {
	}
	...
	public boolean send(GeneratedMessage msg) {
		...
		System.out
				.println("Writing to the outound channel from the ServerAsClient");
	...
	public void handleMessage(eye.Comm.Response msg) {
		if (msg.getHeader().getRoutingId() == Header.Routing.FINGER) {
			...
			logger.info("Searching for the PCQ for corrID::"
					+ msg.getCorrelationId() + " in the routing log"
					+ Server.ownNodeID + ":::"
					+ RoutingLog.getInstance().routingLog);
			if (RoutingLog.getInstance().routingLog.containsKey(msg
					.getCorrelationId())) {
				logger.info("nailed it!...");
				PerChannelQueue pcq = RoutingLog.getInstance().routingLog
						.get(msg.getCorrelationId());
				logger.info("RESPONSE::" + msg.getCorrelationId()
						+ " to be enqueued in outbound queue of " + pcq);
				pcq.enqueueReply(msg);
			}
	...
	public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) {
		logger.info("BINGO! message received in the ServerAsClient handler");
